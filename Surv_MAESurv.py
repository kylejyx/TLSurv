"""
Code for training TTSurv(MAE)
Example usage: python Surv_MAESurv.py mirna mrna epochs learning_rate l2_regularization batch_size
...
DATA_SUFFIX = "_TT"
INDEX_SET = "2"
STATE_FOLDER = os.path.expanduser('~/states_MAE_LUAD_set2/')
SAVE_FOLDER = os.path.expanduser('~/states_MAE_LUAD_set2/')

LOG_FOLDER = os.path.expanduser('./log_MAESurv_set2/')
PREFIX = ""
...
def MAESurv_pipeline(mod1, mod2, x1, x2, y1, y2, hyperparameters, save, path):
    in_dims = hyperparameters['Input dimension']#number of nodes in dense layers
    d_dims = hyperparameters['Embedding dimension']
    hidden_dims = hyperparameters['Latent size'] # number of nodes (dimensionality) of encoded data
    ...
    if mod2 != 'None':
        ...
        net = MAESurv(in_dims, d_dims, hidden_dims, dropout_p, neuron_size).to(device)
    else:
        pass
    ...
    net.eval()
    if save:
        PATH = SAVE_FOLDER + "MAESurv_"+ path
        torch.save(net.state_dict(), PATH+".pt")
    return model, log


def MAESurv_evaluate(model, x1, x2, durations, events):
...
def CV(mod1, mod2, x1, x2, y1, y2, folds, hyperparameters):
    ...
    for train_index, test_index in kf.split(x1):
        ...
        p_time = time.time()
        model, log = MAESurv_pipeline(mod1, mod2, x1_train, x2_train, y1_train, y2_train, hyperparameters, False, None)
        a_time = time.time()
